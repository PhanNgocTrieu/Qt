...
int callbackColName(void* NotUsed, int argc, char** argv, char** azColName)
{
    ...
    std::cout << std::endl;
    std::cout << "  ";
    for (int idex = 0; idex < argc * 20; idex++)
    {
        std::cout << "-";
    }
    printf("\n");
...
void isolated::setDir(const std::string& dirToDB)
{
    this->dirToDB = dirToDB;
}

std::string isolated::getDir()
{
    return this->dirToDB;
}
...
void isolated::insertValuesAuto()
{
	...
	for (int idex = 0; idex < 4; idex++)
	{
		...
		isolatedPlaces.area_ID = isolated::isolatedID;
...
void isolated::increasingIsolated()
{
	isolated::isolatedID++;
}

int isolated::getIsolatedID()
{
	return isolated::isolatedID;
}
